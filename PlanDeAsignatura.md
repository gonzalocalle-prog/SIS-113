...
Programación II es el curso donde el estudiante consolida la Programación Orientada a Objetos (POO) en C# -clases, herencia, polimorfismo, encapsulamiento y concurrencia- como base para asignaturas y proyectos posteriores. Esto se mantiene sin cambios respecto al plan original.
...
- Sesión 3 - **migración guiada a Codespaces**: se muestra en vivo la limitación de WebVM (`dotnet` no instala por ser 32 bits) y cada estudiante abre su primer Codespace (Ubuntu + SDK de .NET) a partir de la plantilla del curso, repitiendo ahí los mismos comandos ya dominados. Cierre con `dotnet --version` y un "Hello World".

### 3.2 Puente hacia el entorno de C# (GitHub Codespaces)

- Plantilla del curso: repositorio con `.devcontainer/devcontainer.json` (imagen `mcr.microsoft.com/devcontainers/dotnet`) + `README.md` con instrucciones.
...
| | 3 | Procesamiento de texto, scripting, migración a Codespaces. | Agente CLI (ámbar) + puente al entorno con .NET. |
...
| | 16-17 | Task, async/await, hilos. Sincronización. Cierre del proyecto final. | Uso agéntico habilitado (ámbar/verde), con bitácora y defensa técnica final. |
...
**Base (se mantiene del plan original)**
...
- Microsoft. (2023). *Object-oriented programming (C#)*. Microsoft Learn. https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/tutorials/oop
